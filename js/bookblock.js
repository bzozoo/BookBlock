	...
	BookBlock.prototype = {
		...
		_initEvents: function () {
			...
			if (this.options.nextEl !== "") {
				document
					.querySelector(this.options.nextEl)
			...
			if (this.options.prevEl !== "") {
				document
					.querySelector(this.options.prevEl)
					.addEventListener("click", function () {
						self._action("prev");
						return false;
					});
			...
			if (this.options.firstEl !== "") {
				document
					.querySelector(this.options.firstEl)
					.addEventListener("click", function () {
						self.first();
						return false;
					});
			...
			if (this.options.lastEl !== "") {
				document
					.querySelector(this.options.lastEl)
					.addEventListener("click", function () {
						self.last();
						return false;
					});
		...
		_layoutNoSupport: function (dir) {
			...
			var isLimit =
				(dir === "next" && this.currentIdx === this.itemsCount - 1) ||
				(dir === "prev" && this.currentIdx === 0);
			// callback trigger
			...
			s_middle.addEventListener(this.transEndEventName, function (event) {
				if (
					...
					Array.prototype.slice
						.call(self.el.querySelectorAll(".bb-page"))
					...
					var isLimit =
						(dir === "next" && self.currentIdx === self.itemsCount - 1) ||
						(dir === "prev" && self.currentIdx === 0);
					// callback trigger
			...
			switch (side) {
				case "left":
					/*
						...
						*/
					var inner =
						dir === "next" ? this.current.innerHTML : this.nextItem.innerHTML;
					sideEl.innerHTML =
				...
				case "middle":
					...
					var frontinner =
						dir === "next" ? this.current.innerHTML : this.nextItem.innerHTML;
					var backinner =
						dir === "next" ? this.nextItem.innerHTML : this.current.innerHTML;
					sideEl.innerHTML =
		...
		destroy: function () {
			...
			if (this.options.firstEl !== "") {
				this.options.firstEl.removeEventListener("click");
				this.options.firstEl.removeEventListener("touchstart");
			}

			if (this.options.lastEl !== "") {
				this.options.lastEl.removeEventListener("click");
				this.options.lastEl.removeEventListener("touchstart");
			}
